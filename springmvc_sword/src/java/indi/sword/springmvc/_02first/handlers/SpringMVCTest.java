...
*/
     ...
     *
     *      ②. 确定 target 属性:
     * 	    在 implicitModel 中查找 attrName 对应的属性值. 若存在, ok 若不存在: 则验证当前 Handler 是否使用了 @SessionAttributes 进行修饰,
     * 	    若使用了, 则尝试从 Session 中 获取 attrName 所对应的属性值. 若 session 中没有对应的属性值, 则抛出了异常.
     ...
     * 2). SpringMVC 会 value 为 key, POJO 类型的对象为 value, 存入到 request 中.
     */
    @ModelAttribute
    public void getUser(@RequestParam(value="id",required=false) Integer id,
        ...
        if(id != null){
            //模拟从数据库中获取对象
            User user = new User(1, "Tom", "123456", "tom@ljb.com", 12);
    ...
    public String testSessionAttributes(Map<String, Object> map){
        User user = new User("Tom", "123456", "tom@ljb.com", 15);
        map.put("user", user);
        map.put("school", "ljb");
        return SUCCESS;
    }
...
//		return SUCCESS;
    ...
    @RequestMapping("/testAntPathSingle/?/abc")
    public String testAntPathSingle() {
        System.out.println("testAntPath... ?");
        return SUCCESS;
    }

    @RequestMapping("/testAntPathX/*/abc")
    public String testAntPathX() {
        System.out.println("testAntPath... * ");
        return SUCCESS;
    }

    @RequestMapping("/testAntPathXX/**/abc")
    public String testAntPathXX() {
        System.out.println("testAntPath... **");
        return SUCCESS;
    }
